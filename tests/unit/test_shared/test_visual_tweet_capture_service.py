...
import src.shared.visual_tweet_capture_service as vtcs_module
from src.shared.visual_tweet_capture_service import VisualTweetCaptureService, capture_twitter_account_visuals
...
class TestVisualTweetCaptureService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tweet_fetcher_patcher = patch('src.shared.visual_tweet_capture_service.TweetFetcher')
        ...
        self.s3_client_patcher = patch('src.shared.visual_tweet_capture_service.boto3.client')
        ...
        # Mock tempfile
        self.tempfile_patcher = patch('src.shared.visual_tweet_capture_service.tempfile.mkdtemp')
        self.mock_tempfile = self.tempfile_patcher.start()
        self.mock_tempfile.return_value = "/tmp/test_capture"
        
        # Mock shutil for cleanup
        self.shutil_patcher = patch('src.shared.visual_tweet_capture_service.shutil.rmtree')
...
class TestRetryMechanism(TestVisualTweetCaptureService):
    ...
    @patch('src.shared.visual_tweet_capture_service.webdriver.Chrome')
    @patch('src.shared.visual_tweet_capture_service.ChromeDriverManager')
    @patch('src.shared.visual_tweet_capture_service.Service')
    ...
    @patch('src.shared.visual_tweet_capture_service.ChromeDriverManager')
    @patch('src.shared.visual_tweet_capture_service.Service')
    @patch('src.shared.visual_tweet_capture_service.time.sleep')  # Speed up tests
    ...
    @patch('src.shared.visual_tweet_capture_service.ChromeDriverManager')
    @patch('src.shared.visual_tweet_capture_service.Service')
    @patch('src.shared.visual_tweet_capture_service.time.sleep')
...
class TestPageNavigationRetry(TestVisualTweetCaptureService):
    ...
    def setUp(self):
        ...
        # Mock WebDriverWait
        self.wait_patcher = patch('src.shared.visual_tweet_capture_service.WebDriverWait')
        ...
        self.sleep_patcher = patch('src.shared.visual_tweet_capture_service.time.sleep')
...
class TestConvenienceFunction(TestVisualTweetCaptureService):
    ...
    @patch('src.shared.visual_tweet_capture_service.VisualTweetCaptureService')
...
class TestRetryConfiguration(TestVisualTweetCaptureService):
    ...
    def test_exponential_backoff_calculation(self, mock_sleep):
        ...
        with patch('src.shared.visual_tweet_capture_service.webdriver.Chrome') as mock_chrome:
            mock_chrome.side_effect = WebDriverException("connection timeout")
            with patch('src.shared.visual_tweet_capture_service.ChromeDriverManager'):
                with patch('src.shared.visual_tweet_capture_service.Service'):
